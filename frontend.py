...
function toolCard(t){const r=t.result||{},meta=r.error?'error':`${r.row_count_returned??0} row${r.row_count_returned===1?'':'s'}${r.truncated?' shown (truncated)':''}`;return `<details class="tool"><summary>SQL query · ${meta}</summary><div class="tool-body"><span class="tool-label">Query</span><pre>${esc(t.query)}</pre><span class="tool-label">Response</span><pre>${esc(JSON.stringify(r,null,2))}</pre></div></details>`;}
        ...
        try:
            ...
            def report_tool_call(trace):
                ...
                self.send_event("status", {
                    "message": f"SQL query {len(tools)} complete · interpreting the result",
                })
